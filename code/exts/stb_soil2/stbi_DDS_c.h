...
TextureInfo* stbi__dds_info_from_memory(stbi_uc const *buffer, int len)
{
	stbi__context s;
	stbi__start_mem(&s,buffer, len);
...
TextureInfo* stbi__dds_info_from_callbacks(stbi_io_callbacks const *clbk, void *user)
{
	stbi__context s;
	stbi__start_callbacks(&s, (stbi_io_callbacks *) clbk, user);
	return stbi__dds_info( &s);
...
#ifndef STBI_NO_STDIO
TextureInfo* stbi__dds_info_from_path(char const *filename)
{
   TextureInfo* res = NULL;
   FILE *f = fopen(filename, "rb");
   if (!f) return 0;
   res = stbi__dds_info_from_file( f);
   fclose(f);
   return res;
}

TextureInfo* stbi__dds_info_from_file(FILE *f)
{
   stbi__context s;
   TextureInfo* res;
   long n = ftell(f);
   stbi__start_file(&s, f);
   res = stbi__dds_info(&s);
   fseek(f, n, SEEK_SET);
   return res;
}
#endif

static TextureInfo* stbi_dds_decompress(stbi__context *s, stbi_uc *dds_data, unsigned int fourCC, int cubemap_faces, int mipCount)
{
	/*	compressed	*/
	//	note: header.sPixelFormat.dwFourCC is something like (('D'<<0)|('X'<<8)|('T'<<16)|('1'<<24))
	int DXT_family = 1 + (fourCC >> 24) - '1';
	int block_size = DXT_family == 1 ? 8 : 16;
	int block_pitch = (s->img_x + 3) >> 2;
	int num_blocks = block_pitch * ((s->img_y + 3) >> 2);
	...
	int sz = (s->img_x)*(s->img_y) * 4 * cubemap_faces;
...
static TextureInfo* stbi__dds_load(stbi__context *s, int req_comp)
{
	...
	int comp = s->img_n;
	int block_size = 16;
	s->img_n = has_alpha ? 4 : 3;
	comp = s->img_n;
	/*	is this uncompressed?	*/
	if (is_compressed)
	{
		fseek((FILE*)s->io_user_data, 0, SEEK_END);
		sz = ftell((FILE*)s->io_user_data) - 128;
		fseek((FILE*)s->io_user_data, 128, SEEK_SET);
		dds_data = (unsigned char*)malloc(sz);
		stbi__getn(s, dds_data, sz);
	}
	else
	{
		/*	uncompressed	*/
...
#ifndef STBI_NO_STDIO
TextureInfo *stbi__dds_load_from_file(FILE *f, int req_comp)
{
	stbi__context s;
	stbi__start_file(&s,f);
	return stbi__dds_load(&s,req_comp);
}

TextureInfo *stbi__dds_load_from_path(const char *filename, int req_comp)
{
   void *data;
   FILE *f = fopen(filename, "rb");
   if (!f) return NULL;
   data = stbi__dds_load_from_file(f,req_comp);
   fclose(f);
   return data;
}
#endif

TextureInfo *stbi__dds_load_from_memory(stbi_uc const *buffer, int len, int req_comp)
{
	stbi__context s;
   stbi__start_mem(&s,buffer, len);
   return stbi__dds_load(&s,req_comp);
...
TextureInfo *stbi__dds_load_from_callbacks(stbi_io_callbacks const *clbk, void *user, int req_comp)
{
	stbi__context s;
   stbi__start_callbacks(&s, (stbi_io_callbacks *) clbk, user);
   return stbi__dds_load(&s,req_comp);
