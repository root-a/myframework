	...
	)
{
	...
	ti = SOIL_load_image(filename, force_channels);
	if( ti == NULL )
	{
	...
	else
	{
		...
		{
			ti->numOfElements = force_channels;
		}
	}

	/*	OK, make it a texture!	*/
	tex_id = SOIL_internal_create_OGL_texture(
			ti->data, &ti->width, &ti->height, ti->numOfElements,
	...
	)
{
	...
	{
		RGBE_to_RGBdivA(ti->data, ti->width, ti->height, rescale_to_max);
	} else if( fake_HDR_format == SOIL_HDR_RGBdivA2 )
	{
		RGBE_to_RGBdivA2(ti->data, ti->width, ti->height, rescale_to_max);
	}
	/*	OK, make it a texture!	*/
	tex_id = SOIL_internal_create_OGL_texture(
			ti->data, &ti->width, &ti->height, ti->numOfElements,
	...
	)
{
	...
	/*	try to load the image	*/
	ti = SOIL_load_image_from_memory(
					buffer, buffer_length,
					force_channels );
	...
	)
{
	...
	ti = SOIL_load_image(x_pos_file, force_channels);
	...
	{
		/*	1st face: try to load the image	*/
		ti = SOIL_load_image(x_neg_file, force_channels);
	...
	{
		/*	1st face: try to load the image	*/
		ti = SOIL_load_image( y_pos_file, force_channels );
	...
	{
		/*	1st face: try to load the image	*/
		ti = SOIL_load_image(y_neg_file, force_channels);
	...
	{
		/*	1st face: try to load the image	*/
		ti = SOIL_load_image(z_pos_file, force_channels);
	...
	{
		/*	1st face: try to load the image	*/
		ti = SOIL_load_image(z_neg_file, force_channels);
	...
	)
{
	...
	if( (ti->width != 6* ti->height) &&
		(6* ti->width != ti->height) )
	{
		SOIL_free_texture_info( ti );
...
TextureInfo*
	SOIL_load_image(const char *filename, int force_channels)
{
	TextureInfo *result = stbi_load(filename, force_channels);
	if( result == NULL )
	{
		result_string_pointer = stbi_failure_reason();
	} else
...
TextureInfo*
	SOIL_load_imagef(const char *filename, int force_channels)
{
	TextureInfo *result = stbi_loadf(filename, force_channels);
	if (result == NULL)
	{
		result_string_pointer = stbi_failure_reason();
	}
...
{
	TextureInfo *result = stbi_load_from_memory(
				buffer, buffer_length,
				force_channels );
	if( result == NULL )
	{
...
void SOIL_free_texture_info(TextureInfo *ti)
{
	STBI_FREE_TI(ti);
}
