...
static TextureInfo* stbi__pvr_info(stbi__context *s)
{
	...
	int x = s->img_x = header.dwWidth;
	int y = s->img_y = header.dwHeight;
	int comp = s->img_n = ( header.dwBitCount + 7 ) / 8;
	...
	PVRExtraInfo* ei = malloc(sizeof(PVRExtraInfo));
	ei->compressed = iscompressed;
...
TextureInfo* stbi__pvr_info_from_memory(stbi_uc const* buffer, int len)
{
	stbi__context s;
	stbi__start_mem(&s, buffer, len);
...
TextureInfo* stbi__pvr_info_from_callbacks(stbi_io_callbacks const *clbk, void *user)
{
	stbi__context s;
	stbi__start_callbacks(&s, (stbi_io_callbacks *)clbk, user);
	return stbi__pvr_info( &s);
...
#ifndef STBI_NO_STDIO
TextureInfo* stbi__pvr_info_from_path(char const *filename)
{
   FILE *f = fopen(filename, "rb");
   if (!f) return 0;
   TextureInfo* res = stbi__pvr_info_from_file(f);
   fclose(f);
   return res;
}

TextureInfo* stbi__pvr_info_from_file(FILE *f)
{
   stbi__context s;
   long n = ftell(f);
   stbi__start_file(&s, f);
   TextureInfo* res = stbi__pvr_info(&s);
   fseek(f, n, SEEK_SET);
   return res;
}
#endif
...
#ifndef STBI_NO_STDIO
TextureInfo* stbi__pvr_load_from_file(FILE *f, int req_comp)
{
	stbi__context s;
	stbi__start_file(&s,f);
	return stbi__pvr_load(&s, req_comp);
}

TextureInfo* stbi__pvr_load_from_path(char const*filename, int req_comp)
{
	FILE *f = fopen(filename, "rb");
	if (!f) return NULL;
	TextureInfo* res = stbi__pvr_load_from_file(f, req_comp);
	fclose(f);
	return res;
}
#endif

TextureInfo* stbi__pvr_load_from_memory (stbi_uc const *buffer, int len, int req_comp)
{
	stbi__context s;
	stbi__start_mem(&s, buffer, len);
	return stbi__pvr_load(&s, req_comp);
...
TextureInfo* stbi__pvr_load_from_callbacks (stbi_io_callbacks const *clbk, void *user, int req_comp)
{
	stbi__context s;
	stbi__start_callbacks(&s, (stbi_io_callbacks *)clbk, user);
	return stbi__pvr_load(&s, req_comp);
