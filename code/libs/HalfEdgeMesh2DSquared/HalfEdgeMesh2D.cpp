		...
		for (int x = 0; x < width; x++)
		{
			...
			else
			{
				...
				if (x > 0)
				{
					...
					if (map[cellToTheLeft] != 'X')
					{
						//pair with the left one
						Edge* lastFaceEdge = faces.back()->edge->next->next;
				...
				if (y > 0)
				{
					...
					if (map[cellAbove] != 'X')
					{
						...
						Edge* aboveFaceEdge = allFaces[cellAbove]->edge->next->next->next;
				...
				newEdge1->next->next->next->face = newFace;
...
bool HalfEdgeMesh2D::isPointInNode(Vector2 point, Face* node)
{
	...
	do
	{
		...
		if (vectorToPoint.vect[0] != 0.f && vectorToPoint.vect[1] != 0.f)
		{
		...
		if (sideVectorToEdge.dotAKAscalar(vectorToPoint) > 0.f)
		{
			return false;
		}
